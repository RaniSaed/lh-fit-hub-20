...
interface CalendarStripProps {
  ...
  progressDates?: string[]; // ISO Strings or YYYY-MM-DD
}

const CalendarStrip: React.FC<CalendarStripProps> = ({ selectedDate, onSelectDate, workoutDates, progressDates = [] }) => {
        ...
        {datesList.map((d, i) => {
          ...
          return (
            ...
            >
              ...
              {/* Indicators */}
              <div className="h-2 w-full flex items-center justify-center mt-1 gap-1">
                {hasWorkout && (
                  <span className={`w-1.5 h-1.5 rounded-full ${isActive ? 'bg-white' : 'bg-[#B0E0E6]'}`} />
                )}
                {hasProgress && (
                  <span className={`w-1.5 h-1.5 rounded-full ${isActive ? 'bg-white/80' : 'bg-[#FF69B4]'}`} />
                )}
              </div>
            </motion.button>
          );
